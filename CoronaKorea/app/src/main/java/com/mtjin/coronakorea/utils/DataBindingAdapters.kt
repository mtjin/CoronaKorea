...
fun AppCompatSpinner.setSelectedListener(viewModel: CityViewModel) {
    onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
        ...
        override fun onItemSelected(p0: AdapterView<*>?, p1: View?, position: Int, p3: Long) {
            when (position) {
                0 -> viewModel.requestKorea()
                1 -> viewModel.requestCity("seoul")
                2 -> viewModel.requestCity("busan")
                3 -> viewModel.requestCity("chungbuk")
                4 -> viewModel.requestCity("chungnam")
                5 -> viewModel.requestCity("daegu")
                6 -> viewModel.requestCity("daejeon")
                7 -> viewModel.requestCity("gangwon")
                8 -> viewModel.requestCity("gwangju")
                9 -> viewModel.requestCity("gyeongbuk")
                10 -> viewModel.requestCity("gyeonggi")
                11 -> viewModel.requestCity("gyeongnam")
                12 -> viewModel.requestCity("incheon")
                13 -> viewModel.requestCity("jeju")
                14 -> viewModel.requestCity("jeonbuk")
                15 -> viewModel.requestCity("jeonnam")
                16 -> viewModel.requestCity("sejong")
                17 -> viewModel.requestCity("ulsan")
            }
        }
    }
}
